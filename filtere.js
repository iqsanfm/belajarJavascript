...
const books = [
  ...
  const filtered = books.filter((item) => item.sales > 1000000)
  const greatAuthors = filtered.map((item) => {return `${item.author} adalah penulis buku ${item.title} yang sangat hebat!`});
